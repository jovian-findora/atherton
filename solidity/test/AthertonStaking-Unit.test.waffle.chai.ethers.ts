...
describe('Atherton Staking', () => {
    ...
    beforeEach(async () => {
        ...
        Staking = await ethers.getContractFactory('AthertonStaking');
        ...
        await staking.initialize( ather.address, soly.address, treasury.address, calculateEpoch.address);
        ...
        await ather.mint(deployer.address, '10000000000000');
        await ather.mint(addr1.address, '10000000000000000');
        // await ather.mint(addr3.address, 1000000000000000);

        await ather.transferOwnership(treasury.address);

        nonce = await ather.nonces(deployer.address)
        nonceAddr1 = await ather.nonces(addr1.address);

        deadline = ethers.constants.MaxUint256

        domain = {
            name: await ather.name(),
            version:'1',
            chainId: deployer.provider._network.chainId,
            verifyingContract: ather.address.toString()
        }
    ...
    describe('stakeOLY()', () => {
        ...
        it('should not let a user stake if they are not the proper signer', async () => {
            const _nonce = await ather.nonces(deployer.address);
        ...
        it('should not let a user stake more than they have', async () => {
            ...
            expect(await soly.balanceOf(deployer.address)).to.equal(0);
            expect(await ather.balanceOf(deployer.address)).to.equal('10000000000000');
        ...
        it('should transfer ATHER from staker to staking contract when stake is made', async () => {
            const balanceBefore = await ather.balanceOf(deployer.address);

            await staking.stakeOLY(tokenAmount, deadline,  sig.v, sig.r, sig.s);

            const balanceAfter = await ather.balanceOf(deployer.address);
    ...
    describe('unstakeOLY()', () => {
        ...
        it('should transfer ATHER back to user when they unstake', async () => {
            ...
            await staking.unstakeOLY('500000000', deadline, _sig.v, _sig.r, _sig.s);
            expect(await ather.balanceOf(deployer.address)).to.equal(+balanceAfter.toNumber() + +500000000);
            ...
            await staking.unstakeOLY(500000000, deadline, _sig2.v, _sig2.r, _sig2.s);
            expect(await ather.balanceOf(deployer.address)).to.equal(balanceAfter.toNumber() + 1000000000);
        });

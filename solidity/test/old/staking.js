...
//         staking = await Staking.deploy(ather.address);

//         Treasury = await ethers.getContractFactory('MockTreasury');
//         treasury = await Treasury.deploy();

//         await treasury.setStakingAndOLY(staking.address, ather.address);

//         await ather.mint(treasury.address, 9000000000000000);

//         sOLY = await ethers.getContractFactory('sAtherton');
//         soly = await sOLY.deploy('sAtherton', 'sOLY', staking.address);
    
//         await staking.setSOLY(soly.address);
//         await staking.setOwner(treasury.address)
//         await soly.setMonetaryPolicy(staking.address);

//         await ather.mint(addr1.address, 1000000000000000);
//         await ather.mint(addr2.address, 1000000000);
//         await ather.mint(addr3.address, 1000000000000000);
//         await ather.approve(staking.address, 1000000000000000);
//         await soly.approve(staking.address, 1000000000000000);
//         await ather.connect(addr1).approve(staking.address, 1000000000000000);
//         await ather.connect(addr2).approve(staking.address, 1000000000000000);
//         await ather.connect(addr3).approve(staking.address, 1000000000000000);
...
//             expect(await ather.balanceOf(addr2.address)).to.equal(1000000000);
//         });
...
//         it('should transfer ATHER from staker to staking contract when stake is made', async () => {
//             const balanceBefore = await ather.balanceOf(owner.address);

//             await staking.stakeOLY(1000000000);

//             const balanceAfter = await ather.balanceOf(owner.address);
...
//         it('should transfer ATHER back to user when they unstake', async () => {
//             await staking.stakeOLY(1000000000);

//             const balanceAfter = await ather.balanceOf(owner.address);

//             await staking.unstakeOLY(500000000);
//             expect(await ather.balanceOf(owner.address)).to.equal(balanceAfter.toNumber() + 500000000);

//             await staking.unstakeOLY(500000000);
//             expect(await ather.balanceOf(owner.address)).to.equal(balanceAfter.toNumber() + 1000000000);
//         });
...
//         it('should send ATHER back to forfeiter if they have not claimed any', async () => {
//             const balanceBefore = await ather.balanceOf(owner.address);
//             await staking.stakeOLY(1000000000);
...
//             await staking.forfeit();
//             const balanceAfterForfeit = await ather.balanceOf(owner.address);
//             expect(balanceBefore).to.equal(balanceAfterForfeit);
//         });

//         it('should send ATHER back to forfeiter if they have claimed some', async () => {
//             const balanceBefore = await ather.balanceOf(owner.address);
//             await staking.stakeOLY(1000000000);
...
//             await staking.unstakeOLY(500000000);
//             expect(await ather.balanceOf(owner.address)).to.equal(balanceBefore - 500000000);

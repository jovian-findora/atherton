...
export const ATHERPrice = () => {
  const marketPrice = useSelector(state => state.app.marketPrice);

  return (
    <Metric className="price">
      <Metric.Title>ATHER Price</Metric.Title>
...
export const CurrentIndex = () => {
  ...
  return (
    <Metric className="index">
      <Metric.Title>
        Current Index
        <InfoTooltip message="The current index tracks the amount of sATHER accumulated since the beginning of staking. Basically, how much sATHER one would have if they staked and held a single ATHER from day 1." />
      </Metric.Title>
      <Metric.Value>{currentIndex && trim(currentIndex, 2) + " sATHER"}</Metric.Value>
    </Metric>
  );
};

export const WSATHERPrice = () => {

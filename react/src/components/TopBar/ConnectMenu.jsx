...
import { getTxViewUrl, useWeb3Context } from "src/hooks/web3Context";
...
function ConnectMenu({ theme }) {
  ...
  const id = open ? "ather-popper-pending" : undefined;
            ...
            <Fade {...TransitionProps} timeout={100}>
              <Paper className="ather-menu" elevation={1}>
                {pendingTransactions.map((x, i) => (
                  <Box key={i} fullWidth>
                    <Link key={x.txnHash} href={getTxViewUrl(x.txnHash)} target="_blank" rel="noreferrer">

...
export class EnvHelper {
  ...
  static alchemyTestnetURI = `https://eth-rinkeby.alchemyapi.io//${EnvHelper.env.REACT_APP_TESTNET_ALCHEMY}`;
  ...
  static getAlchemyAPIKeyList() {
    ...
    if (ALCHEMY_ID_LIST.length > 0) {
      ALCHEMY_ID_LIST = ALCHEMY_ID_LIST.map(alchemyID => `https://eth-mainnet.alchemyapi.io//${alchemyID}`);
    } else {
      ALCHEMY_ID_LIST = [];
    }
  ...
  static getAPIUris() {
    ...
    if (EnvHelper.env.NODE_ENV === "development" && ALL_URIs.length === 0) {
      ...
      ALL_URIs.push("https://eth-mainnet.alchemyapi.io//_gg7wSSi0KMBsdKnGVfHDueq6xMB9EkC");
    }

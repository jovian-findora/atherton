...
import { ReactComponent as AtherImg } from "../assets/tokens/token_ATHER.svg";
import { ReactComponent as SAtherImg } from "../assets/tokens/token_sATHER.svg";

// import { ather_dai } from "./AllBonds";
import { JsonRpcSigner, StaticJsonRpcProvider } from "@ethersproject/providers";
import { IBaseAsyncThunk } from "src/slices/interfaces";
import { IUniswapV2Pair, RedeemHelper } from "../typechain";

export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  // const ather_dai_address = ather_dai.getAddressForReserve(networkID);
  // const pairContract = new ethers.Contract(ather_dai_address, IUniswapV2PairABI, provider) as IUniswapV2Pair;
 ...
 */
export async function getTokenPrice(tokenId = "atherton") {
  let resp;
  try {
...
function getSatherTokenImage() {
  return <SvgIcon component={SAtherImg} viewBox="0 0 100 100" style={{ height: "1rem", width: "1rem" }} />;
}

export function getAtherTokenImage(w?: number, h?: number) {
  const height = h == null ? "32px" : `${h}px`;
  const width = w == null ? "32px" : `${w}px`;
  return <SvgIcon component={AtherImg} viewBox="0 0 32 32" style={{ height, width }} />;
}

export function getTokenImage(name: string) {
  if (name === "ather") return getAtherTokenImage();
  if (name === "sather") return getSatherTokenImage();
}
...
export const shouldTriggerSafetyCheck = () => {
  ...
  const _safetyCheckKey = "-ather-safety";

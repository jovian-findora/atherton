...
let mainNetwork: NetworkInfo = null;

export const FINDORA_FORGE_TESTNET = {
  chainId: '0x20D',
  chainName: 'Findora Forge Testnet',
  nativeCurrency: {
      name: 'Findora',
      symbol: 'FRA',
      decimals: 18,
...
const isProduction = !window.location.hostname.startsWith('forge.');

mainNetwork = isProduction ? FINDORA_MAINNET : FINDORA_FORGE_TESTNET;
...
const ALL_URIs = mainNetwork.rpcUrls;
...
export const enforceNetwork = () => {
  return new Promise<boolean>(resolve => {
    if (window.ethereum && !(window as any).ethereumNetworkBeingRequested) {
      (window as any).ethereumNetworkBeingRequested = true;
      window.ethereum.request({
        method: 'wallet_addEthereumChain',
        params: [mainNetwork],
      }).then(() => {
        (window as any).ethereumNetworkBeingRequested = false;
        resolve(true);
      }).catch((error: any) => {
        (window as any).ethereumNetworkBeingRequested = false;
        console.log(error);
...
export const getTxViewUrl = (txnHash: string) => {
  if (mainNetwork) {
    let explorerUrl = mainNetwork.blockExplorerUrls[0];
    while (explorerUrl.endsWith('/')) { explorerUrl = explorerUrl.slice(0, -1); }
    return `${explorerUrl}/tx/${txnHash}`;
  } else {
    return null;
  }
};
...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const [chainID, setChainID] = useState(chainIdInt[mainNetwork.chainId]);
    ...
    new Web3Modal({
      ...
      providerOptions: {
        walletconnect: {
          ...
          options: {
            rpc: {
              [mainNetwork.chainId]: mainNetwork.rpcUrls[0],
            },
          },
        },
      },
  ...
  const _checkNetwork = (otherChainID: number): boolean => {
    if (typeof otherChainID === 'string') {
      otherChainID = parseInt(otherChainID, 10);
    }
    if (chainID !== otherChainID) {
      console.warn("You are switching networks");
      if (otherChainID === chainIdInt[mainNetwork.chainId]) {
  ...
  const connect = useCallback(async () => {
    ...
    if (!validNetwork) {
      console.error(`Wrong network, please switch to '${mainNetwork.chainName}'`);
      enforceNetwork();
      return;
    }
